...

{
  ...
  if( inherits(y,"factor") || inherits(y,"character") )
  {
    ...
    #data[,y.clm]=(data[,y.clm]==uniq.y[i]) #AO: assuming i should be  1 here
    data[,y.clm]=(data[,y.clm]==uniq.y[1])
    res=model.binary(form,data,tbl,fig,txt,clr)
    return(res)
  }
...

{
  ...
  {
    cox.res=survival::coxph(form,data)
    ...
    if (fig>0)
    {
      grp=cut(y.hat,c(-Inf,stats::quantile(y.hat,(1:2)/3,na.rm=T),Inf))
      srv=cox.res$y
  ...
  {
    mdl.frm=stats::model.frame(form,data,na.action=stats::na.omit)
...
{
  glm.res=stats::glm(formula=form,data=data,family=stats::binomial)
  res.tbl=glm.tbl(glm.res)

  r=stats::residuals(glm.res)
  y.hat=stats::predict(glm.res)
  p.hat=exp(y.hat)/(1+exp(y.hat))
  y.obs=glm.res$y

  if (fig>1)
  {
    graphics::boxplot(p.hat~y.obs,horizontal=T,
...

{
  ...
  print(class(data))
  glm.res=stats::glm(formula=form,data=data,x=T,y=T)
  print("GLM Done")
  res.tbl=glm.tbl(glm.res)
  print("glm.tbl Done")
  ...
  if (fig>2)
  {
    if (nrow(res.tbl)==2)
    {
      ...
      graphics::segments(temp.dset$x,0,
               temp.dset$x,
               temp.dset$r,
               col=clrs[2])
    ...
    if (nrow(res.tbl)>2)
    {
      ...
      graphics::segments(temp.dset$y.hat,
               temp.dset$y.hat,
               temp.dset$y.hat,
               temp.dset$y,
               col=clrs[2])
      ...
      graphics::segments(temp.dset$y.hat,0,
               temp.dset$y.hat,
               temp.dset$r,
               col=clrs[2])
...

{
  ...
  coef.tbl=stats::coef(summary(model.result))
  CI.tbl=stats::confint(model.result)
  ...
  if ((model.class[1]=="glm"))
  {
    ...
    r=stats::residuals(model.result)
  ...
  if (model.type=="linear")
  {
    rsq.value=1-sum(stats::residuals(model.result)^2)/sum((model.result$y-mean(model.result$y,na.rm=T))^2,na.rm=T)
...
{
  fit.res=survival::coxph(form,data,y=T,x=T)
  zph.res=survival::cox.zph(fit.res,terms=F,global=F)
  est.tbl=stats::coef(summary(fit.res))
  ci.tbl=stats::confint(fit.res)
  ...
  zph.gbl=survival::cox.zph(fit.res,global=T)
...
{
  est.tbl=stats::coef(summary(coxph.result))
  ci.tbl=stats::confint(coxph.result)
  ...
  zph.res=survival::cox.zph(coxph.result)
...
{
  ci.tbl=stats::confint(glm.result)
  smry.tbl=stats::coef(summary(glm.result))
...
monotone.rank.model=function(x,y)
{
  ...
  y.hat=ry.hat
  ord.y=order(y)
  y.hat[ord.y]=stats::approx(ry[ord.y],y[ord.y],xout=ry.hat[ord.y])$y
